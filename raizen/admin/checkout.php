		...
		<div class = "panel panel-default">
			...
			<div class = "panel-body">
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `transaction` NATURAL JOIN `guest` NATURAL JOIN `tour` WHERE `status` = 'Check Out'") or die(mysqli_query());
							while($fetch = $query->fetch_array()){
						?>
						<tr>
							<td><?php echo $fetch['firstname']." ".$fetch['lastname']?></td>
							<td><?php echo $fetch['tour_type']?></td>
							<td><?php echo $fetch['tour_no']?></td>
							...
							<td><?php if($fetch['extra_participant'] == "0"){ echo "None";}else{echo $fetch['extra_participant'];}?></td>
