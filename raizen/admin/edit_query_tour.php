<?php
	...
	if(ISSET($_POST['edit_tour'])){
		...
		if ($_FILES['photo']['size'] > 0) {
			...
			$conn->query("UPDATE `tour` SET `tour_type` = '$tour_type', `no_of_days`='$no_of_days', `price` = '$price', `package_inclusions` = '$package_inclusions', `sites_to_visit` = '$sites_to_visit', `remarks` = '$remarks' WHERE `tour_id` = '$_REQUEST[tour_id]'") or die(mysqli_error());
		}
		...
		echo'<script> window.location="tour.php"; </script> ';
	}
?>